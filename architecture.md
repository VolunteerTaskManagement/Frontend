...
This frontend is a React Single Page Application for managing volunteer tasks. It supports two main users: Volunteers and Coordinators.
...
The project follows a practical, feature-oriented frontend architecture with clear separation of responsibilities and boundaries between its main layers.
...
Layouts are separated from page content, which allows the same page structure to be reused consistently across features.

### hooks

Contains custom hooks that organize asynchronous and feature-specific logic in reusable units.
...
Axios instances are centralized in the service layer. Components communicate with the backend through services or custom hooks.
...
This keeps shared feature state organized in focused stores, with each store responsible for a specific feature.
...
The code entry is currently handled as a frontend-controlled step. After four digits are entered, the user can continue through the dedicated confirmation button.
...
Used for shared feature state. Task filters and notification data can be accessed by multiple components, so they are kept in dedicated stores.

This combination assigns each state value to the solution that best matches its scope and keeps feature-specific logic easy to locate.
...
The UI is built from small React components, while pages focus on composing those components into complete screens.
...
This keeps navigation and page structure consistent while allowing each feature component to remain focused on its own responsibility.
...
It also makes endpoint changes easier to manage because service details remain centralized.

### Provider Pattern

AuthProvider exposes authentication state and actions through React Context.

This gives route guards, headers, and pages access to the same session state through a single provider.

### Route Guard Pattern

ProtectedRoute and PublicOnlyRoute centralize access rules.

Route guards centralize token checks in one place, making navigation behavior predictable and keeping page components focused on their own features.

### Feature-Based Stores

Zustand stores are organized by feature, with each store responsible for a focused part of the application state.
...
This centralizes authorization-header logic for every service.
